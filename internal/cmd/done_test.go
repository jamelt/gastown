...
// TestEnsureAgentBeadExists_SingletonRolesUseRigLocalBeads verifies gt-n99t's
// fix: Witness and Refinery are rig-local singleton identities (see
// beads.Beads.ForLocalBeads) and must be recreated/reopened in the rig's own
// database, not the town database that ensureAgentBeadExists's caller
// normally passes. Before the fix, a missing/closed witness or refinery
// agent bead got recreated in town state instead, leaving a stray duplicate
// there while the true rig-local record stayed closed — which is exactly
// what made
// `gt agents resolve --role refinery --rig <rig>` report the bead as
// town-issues-only. Polecats are unaffected: their agent beads intentionally
// live in town state, so they must keep targeting the town database.
func TestEnsureAgentBeadExists_SingletonRolesUseRigLocalBeads(t *testing.T) {
	...
	tests := []struct {
		...
		{"refinery singleton pins to rig db", RoleRefinery, "gt-gastown-refinery", rigBeadsDir, townBeadsDir},
		{"witness singleton pins to rig db", RoleWitness, "gt-gastown-witness", rigBeadsDir, townBeadsDir},
		{"polecat keeps using town db", RolePolecat, "gt-gastown-polecat-shiny", townBeadsDir, rigBeadsDir},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			logPath := filepath.Join(t.TempDir(), "bd.log")
			installEnsureAgentBeadMockBD(t, logPath)
			...
			// Mirrors done.go's real call site: caller always hands in a
			// town-pinned wrapper; ensureAgentBeadExists must re-pin to the
			// rig database itself for singleton roles.
			bd := beads.New(rigDir).ForAgentBead()
			ensureAgentBeadExists(bd, tt.id, ctx)
			...
			if !strings.Contains(logStr, "BEADS_DIR="+tt.wantDir) {
				t.Fatalf("expected a bd invocation with BEADS_DIR=%s; log:\n%s", tt.wantDir, logStr)
			}
			if strings.Contains(logStr, "BEADS_DIR="+tt.wrongDir) {
				t.Fatalf("bd invocation unexpectedly used BEADS_DIR=%s; log:\n%s", tt.wrongDir, logStr)
			}
		})
	}
}

// installEnsureAgentBeadMockBD installs a fake `bd` on PATH that logs the
// BEADS_DIR it was invoked with (one line per call) to logPath, answers
// `show` as not-found, and answers `create` with a stub issue so
// CreateOrReopenAgentBead succeeds without touching a real Dolt server.
func installEnsureAgentBeadMockBD(t *testing.T, logPath string) {
	t.Helper()
	binDir := t.TempDir()
	script := `#!/bin/sh
...
case "$cmd" in
  ...
  show) echo '[]' ;;
  create) echo '{"id":"stub","title":"stub","status":"open"}' ;;
  *) exit 0 ;;
esac
`
