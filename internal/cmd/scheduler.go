...
func runSchedulerRun(cmd *cobra.Command, args []string) error {
	...
	// Enforce hard-prohibition gate marker at scheduler startup per hq-1s4w/gt-i4bn.
	// The binary must contain evidence of gate enforcement before scheduler can start.
...
// The scheduler must refuse to start with a binary that does not contain evidence of gate enforcement.
