...
// TestMailboxArchivePreservesActiveMoleculeGraph is a subprocess-boundary
// regression for gt-3rn. The fake bd models the reported upstream failure by
// destroying an active graph whenever `bd close` is invoked. Mail archival
// must only label the exact validated message IDs, even when one target fails.
func TestMailboxArchivePreservesActiveMoleculeGraph(t *testing.T) {
	...
	workDir := t.TempDir()
	binDir := t.TempDir()
	graphPath := filepath.Join(t.TempDir(), "active-graph")
	logPath := filepath.Join(t.TempDir(), "bd.log")
	wantGraph := "root=active-formula status=hooked hook=deacon\nchildren=26 dependencies=25\n"
	if err := os.WriteFile(graphPath, []byte(wantGraph), 0644); err != nil {
...
case "$1" in
  show)
    case "$2" in
      mail-one|mail-two|mail-three|mail-four|mail-fail)
        printf '[{"id":"%s","title":"mail","description":"body","assignee":"gastown/polecats/nitro","priority":2,"status":"open","created_at":"2026-08-16T20:00:00Z","labels":["gt:message","from:mayor/"]}]\n' "$2"
        ;;
      active-formula)
        printf '[{"id":"active-formula","title":"patrol","description":"","assignee":"deacon/","priority":1,"status":"hooked","created_at":"2026-08-16T19:00:00Z","labels":["gt:formula"]}]\n'
        ;;
      *)
...
`
	...
	t.Setenv("ACTIVE_GRAPH", graphPath)

	mailbox := NewMailboxWithBeadsDir("gastown/polecats/nitro", workDir, beadsDir)
	for _, id := range []string{"mail-one", "mail-two", "mail-three", "mail-four"} {
		if err := mailbox.Archive(id); err != nil {
			t.Fatalf("Archive(%s): %v", id, err)
		}
	}
	if err := mailbox.Archive("active-formula"); !errors.Is(err, ErrNotInboxMessage) {
		t.Fatalf("Archive(active-formula) error = %v, want ErrNotInboxMessage", err)
	}
	if err := mailbox.Archive("mail-fail"); err == nil || errors.Is(err, ErrMessageNotFound) {
		t.Fatalf("Archive(mail-fail) error = %v, want surfaced mutation failure", err)
	}
	...
	if string(graph) != wantGraph {
		t.Fatalf("active molecule graph changed:\n%s", graph)
	}
	...
	if strings.Contains(log, "close ") {
		t.Fatalf("archive invoked destructive close:\n%s", log)
	}
	if strings.Contains(log, "label add active-formula") {
		t.Fatalf("archive mutated non-mail molecule root:\n%s", log)
	}
	for _, id := range []string{"mail-one", "mail-two", "mail-three", "mail-four", "mail-fail"} {
		if got := strings.Count(log, "label add "+id+" archived"); got != 1 {
			t.Fatalf("archive mutations for %s = %d, want 1; log:\n%s", id, got, log)
		}
	}
}
...
if [ "$1" = "list" ]; then
  case "$*" in
    *"--assignee gastown/synth"*)
      printf '%s\n' '[{"id":"issue-direct-open","title":"Direct open","description":"","status":"open","priority":2,"assignee":"gastown/synth","created_at":"2026-06-12T12:00:05Z","labels":["gt:message","from:mayor/"]},{"id":"issue-direct-hooked","title":"Direct hooked","description":"","status":"hooked","priority":2,"assignee":"gastown/synth","created_at":"2026-06-12T12:00:04Z","labels":["gt:message","from:mayor/"]},{"id":"issue-direct-archived","title":"Direct archived","description":"","status":"open","priority":2,"assignee":"gastown/synth","created_at":"2026-06-12T12:00:03Z","labels":["gt:message","from:mayor/","archived"]},{"id":"issue-direct-closed","title":"Direct closed","description":"","status":"closed","priority":2,"assignee":"gastown/synth","created_at":"2026-06-12T12:00:02Z","labels":["gt:message","from:mayor/"]}]'
      exit 0
      ;;
...
`
	...
	wantAbsent := []string{
		"issue-direct-closed",
		"issue-direct-archived",
		"issue-cc-hooked",
		"wisp-direct-archived",
		"wisp-cc-hooked",
	}
