...
var DefaultRateLimitPatterns = []string{
	...
	`(API |HTTP )?Error.*\binsufficient_quota\b`,                       // OpenAI/OpenRouter structured quota error
	`(API |HTTP )?Error.*\brate_limit_exceeded\b`,                      // OpenAI/OpenRouter structured rate-limit error
	`(HTTP\s*)?429\b.*(Too Many Requests|rate.?limit|quota)`,           // Provider HTTP quota response
	`(usage|spend|credit) (limit|quota).*(reached|exhausted|depleted)`, // Terminal allocation exhaustion
}
