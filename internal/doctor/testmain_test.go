...
func TestMain(m *testing.M) {
	testRoot, err := os.MkdirTemp("", "gt-test-doctor-town-*")
	if err != nil {
		fmt.Fprintf(os.Stderr, "doctor TestMain: create isolated town: %v\n", err)
		os.Exit(1)
	}
	beadsDir := filepath.Join(testRoot, ".beads")
	if err := os.MkdirAll(beadsDir, 0o755); err != nil {
		fmt.Fprintf(os.Stderr, "doctor TestMain: create isolated beads dir: %v\n", err)
